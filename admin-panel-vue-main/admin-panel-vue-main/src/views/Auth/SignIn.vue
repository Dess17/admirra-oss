...
const { login, checkAuth, fetchCurrentUser, setToken, getErrorMessage } = useAuth()
...
const handleMaxLogin = async () => {
  ...
  try {
    ...
    errorMessage.value = getErrorMessage(e, e?.message || 'Не удалось войти через MAX')
  } finally {
    oauthLoading.value = false
...
.auth-form-box {
  width: 100%;
  max-width: 386px;
  margin-top: clamp(66px, 10vh, 112px);
}

.auth-title {
  margin: 0 0 27px;
  color: #102a55;
  font-size: clamp(29px, 2.16vw, 40px);
  font-weight: 300;
  line-height: 1.08;
  letter-spacing: 0;
}
